...
const AddFromGitOrg = ({
  ...
  const selectedState = useCallback(() => {
    ...
    if (selectedRepos.length === reposFromOrg.length)
        ...
        <Panel className="h-80">
          <Panel.Header className="justify-between">
            <div className="flex">
              ...
              />
              <HelpText>
                {reposFromOrg.filter((repo) => repo.checked).length} of{' '}
                {reposFromOrg.length} repositories selected
              </HelpText>
            </div>
          ...
          <Panel.Body className="p-0 overflow-y-auto">
            <InlineBanner>
              ...
              <a onClick={openAutoImportView}>Auto import</a> repos from
