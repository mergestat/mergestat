...
const Filter: React.FC<FilterProps> = ({ label, value, defaultValue, reset, explore, overlay }: FilterProps) => {
  ...
  return (
    <div className='flex mr-2 my-1'>
      <Dropdown
        trigger={show
          ? <FilterSelected label={label} value={value} reset={reset} close={() => setShow(false)} />
...
type FilterSelectedProps = PropsWithChildren<{
  label: string
  value?: string | number
  reset: () => void
  close: () => void
...
const FilterSelected: React.FC<FilterSelectedProps> = ({ label, value, reset, close }: FilterSelectedProps) => {
  return (
