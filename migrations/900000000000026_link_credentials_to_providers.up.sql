...
-- add column to store optional username with the token / credential
ALTER TABLE mergestat.service_auth_credentials
...
BEGIN
    INSERT INTO mergestat.service_auth_credentials (provider, type, username, credentials)
        VALUES (provider_id, credential_type, username, pgp_sym_encrypt(token, secret)) RETURNING * INTO _inserted_row;
...
BEGIN
    RETURN QUERY SELECT c.id, c.username, pgp_sym_decrypt(c.credentials, secret) AS token, c.created_at
